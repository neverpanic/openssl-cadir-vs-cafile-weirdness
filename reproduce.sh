...
echo "TEST2.5" | env SSL_CERT_DIR="$(readlink -f ca-trust/certs)" SSL_CERT_FILE="$(readlink -f ca-trust/certs-with-ca.pem)" ADDITIONAL_CA="$(readlink -f otherca.pem)" ADDITIONAL_CADIR="$(readlink -f ca-trust/certs)" ./sslecho c 127.0.0.1
# Broken
